...
namespace AzureDevOpsToPowerBI
{
    ...
    internal static class TaskManager
    {
        internal static async Task<List<TfsTasks>> GetTfsTasks(string projectkey, string projectname, string areapath)
        {
            ...
            string tfsUri = string.Format($"{AppSettings.TfsUri}/{{0}}/_odata/v4.0-preview/WorkItems?$select=WorkItemId,Title,WorkItemType,State,AreaSK,IterationSK,TagNames,ParentWorkItemId,OriginalEstimate,CompletedWork,CompletedDate,CreatedDate,ClosedDate&$filter=WorkItemType eq 'Task' and State ne 'Removed' and startswith(Area/AreaPath,'{{1}}') and CreatedDate ge {{2}} &$orderby=CreatedDate desc",projectname,areapath, AppSettings.WorkItemSyncDate);
